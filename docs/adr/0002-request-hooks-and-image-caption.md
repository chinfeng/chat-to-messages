...
- `/v1/responses` was hook-less initially; it is now covered — see "Update — 2026-09-16" below.
...
## Update — 2026-09-16: hooks on `/v1/responses`

`/v1/responses` converts OpenAI Responses input into chat/completions, and its clients (codex-style) do send `input_image` parts, so the caption hook was extended to it.

- The registry interface stays Anthropic-canonical; the Responses path gets its own application point instead of canonicalizing its input. The caption hook exposes `CaptionItems(items []responses.Item)`, which the shared `buildResponsesUpstream` calls after assembling the `previous_response_id`-expanded history and before `BuildChatBody` — one application point covering both the SSE and websocket transports.
- The hook mutates only the assembled history slice, never `req.Input.Raw` or the store: the STORE keeps the original image-bearing items, so a later turn re-captions from cache, and a model the operator later rules visual replays its images untouched.
- Eviction is not on the Responses path: it predates this hook and caps only Anthropic-canonical messages. So while `/v1/messages` requests are capped to `--max-upstream-images` *before* captioning (eviction registers first), a Responses request's images are captioned with no cap — the batched vision call carries as many `input_image` parts as the request holds. If image-heavy Responses turns ever need bounding, eviction would have to grow a second dialect entry point.
