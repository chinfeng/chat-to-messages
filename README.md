...
Enable `--dump <dir>` to record each downstream request. A session is written to `<dir>/in-progress/<id>/` (where `<id>` is a time-ordered UUID v7) and, on completion, renamed into a classification bucket that reflects the **root cause** of the termination:
